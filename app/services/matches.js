...
module.exports.addMatches = async (req, res) => {
  ...
  if(! ( checkPlayerExist.length == 2 ) ) {
    return common.responseData(res, { status: false, statuscode: 401, message: "Player does not exist with this name" });
  }
  ...
  let matchAdded = await db.collection('matches').insertOne(matchObj);
    ...
    if( matchAdded && matchAdded.insertedCount == 1 )  {
            return common.responseData(res, { status: true, statuscode: 200, message: "match added successfully" });
    }
...
module.exports.getMatches = async (req, res) => {
  ...
  try {
    ...
    let matchesList = await db.collection('matches').find({ }).limit(limit).toArray();
  
    if(!( matchesList && matchesList.length )) {
      return common.responseData(res, { status: false, statuscode: 404, message: "No matches found. Please add one" });
    }
  
    return common.responseData(res, { status: true, statuscode: 200, message: "Players Added successfully" , data: matchesList });
...
module.exports.getTopPerfomers = async (req, res) => {
  ...
  try {
    ...
    return common.responseData(res, { status: true, statuscode: 200, message: "Players Added successfully" , data: isPlayer });
...
async function getTopPerfomersPlayer ( limit ) {
  ...
  let getTopPerformer =  await db.collection('matches').aggregate([
        {$limit : limit },
        {
...
async function isPlayer(obj){
  ...
  const db = getDb();
  console.log("object value is here" , obj);
  let players = await db.collection('players').find({ $or : [{  name : obj.firstPlayerName  }, { name : obj.secondPlayerName  }] }).toArray();
  console.log("players value is here" , players);
